...
def load_cnv_data(file_path, window_size):
    ...
    column_data_types = {'conifer_log2' : 'float64',
                         'chr' : 'str',
                         'start_pos' : 'int64',
                         'end_pos' : 'int64',
                         'gene' : 'str',
    ...
    df['rolling_median'] = df['conifer_log2'].rolling(window_size, min_periods=1).median()
    return df
...
def plot_main(pdf, df, title, min_log_ratio):
    ...
    for chrom in chromosomes:
        ...
        ax.scatter(indices, df_chrom['conifer_log2'], label=chrom, marker=',', s=0.2)
    ...
    plt.ylabel('CoNIFER Log Ratio', fontsize=12)
    ...
    min_log = df['conifer_log2'].min()
    max_log = df['conifer_log2'].max()
...
def plot_gene(pdf, df_gene, transcript=None):
    ...
    ax.scatter(df_introns['mean_pos'], df_introns['conifer_log2'], marker='o', color='black', alpha=0.7, s=3)
    
    # parameters for y-coordinate of exon label
    exon_labels=[]
    gene_median = df_gene['conifer_log2'].median()
    if gene_median > 0:
    ...
    for i, exon in enumerate(covered_exons):
        ...
        s = ax.scatter(df_exon['mean_pos'], df_exon['conifer_log2'], label=exon, marker='o', s=6)
        # add label for exon
        ...
        exon_vectors.append(df_exon['conifer_log2'].values)
    ...
    ax.set_ylabel('CoNIFER Log Ratio', fontsize=12)
    ...
    min_log = df_gene['conifer_log2'].min()
    max_log = df_gene['conifer_log2'].max()
    if min_log < -1 * y_scale or max_log > y_scale:
