...
from genetics_db.database import SqliteConnection, OperationalError
...
def build_parser(parser):
    ...
    parser.add_argument('dbname',
                    help="Name of the database for export")

SEQ_MACHINES={'D00180':{'machine_code':'HA',
                        'server':'narwhal',
...
def run_bcl2fastq(run_info, cores):
    ...
    fastq_output_dir = os.path.join(run_info['drive'],run_info['run_date']+"_"+run_info['machine_code']+run_info['machine_run'])
...
def cat_fastqs(run_info):
    ...
    for project in project_dirs:
        if project.startswith("Project"):
            run_info['projects']=run_info['projects'].append(project)
...
def action(args):
    db = SqliteConnection(args.dbname)
    try:
        db.create(schemas=['sqlite_lis_annotation.sql'])
