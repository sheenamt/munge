...
def flag_genes(df, min_log_ratio, rolling_window_size):
    ...
    """
    flagged_genes = {}
    log2_column_name = df.columns[0]  # allows for flagging conifer or non-conifer log2s

    # apply the rolding median
    df['rolling_median'] = filled_rolling_median(df[log2_column_name], rolling_window_size)
...
def plot_main(pdf, df, title, min_log_ratio, rolling_window_size):
    ...
    if 'conifer_log2' in df.columns:
        ax = fig.add_subplot(211)
        cnf = fig.add_subplot(212, sharex=ax)
        plots = {'log2' : ax, 'conifer_log2' : cnf}
        # label axes
        cnf.set_xlabel('Chromosome', fontsize=12)
        cnf.set_ylabel('CoNIFER Log Ratio', fontsize=12)
    ...
    for chrom in chromosomes:
        ...
        for column_name, axis in plots.items():
            axis.scatter(indices, df_chrom[column_name], label=chrom, marker=',', s=0.2)

    # flag genes that are above or below log ratio threshold
    for column_name, axis in plots.items():
        flagged_df = df[[column_name, 'gene']].copy()
    ...
    for column_name, axis in plots.items():
        min_log = df[column_name].min()
        max_log = df[column_name].max()
...
def plot_gene(pdf, df_gene, transcript=None):
    ...
    if 'conifer_log2' in df_gene.columns and transcript is not None:
        # apportion subplots
    ...
    elif 'conifer_log2' in df_gene.columns:
        # apportion subplots
    ...
    for column_name, axis in plots.items():
        ...
        axis.scatter(df_introns['mean_pos'], df_introns[column_name], marker='o', color='black', alpha=0.7, s=3)

        # parameters for y-coordinate of exon label
        exon_labels=[]
        gene_median = df_gene[column_name].median()
        if gene_median > 0:
        ...
        for i, exon in enumerate(covered_exons):
            ...
            s = axis.scatter(df_exon['mean_pos'], df_exon[column_name], label=exon, marker='o', s=6)
            ...
            exon_vectors.append(df_exon[column_name].values)

        plot_labels[column_name] = exon_labels
    ...
    for column_name, axis in plots.items():
        min_log = df_gene[column_name].min()
        max_log = df_gene[column_name].max()
        ...
        if min_log < -1 * y_scale or max_log > y_scale:
            ...
            for label in plot_labels[column_name]:
